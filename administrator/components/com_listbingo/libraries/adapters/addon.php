...
class JInstallerAddon extends JObject
{
	...
	function install()
	{
		...
		if ($result === false) {
			...
			$this->parent->abort(JText::_('Addon').' '.JText::_('Install').': '.JText::_('SQL Error')." ".$db->stderr(true));
			return false;
		} elseif ($result === 0) {
			...
			if ($utfresult === false) {
				// Install failed, rollback changes
				$this->parent->abort(JText::_('Addon').' '.JText::_('Install').': '.JText::_('SQLERRORORFILE')." ".$db->stderr(true));
				return false;
			}
		}
		...
		if ($id) {
			...
			{
				// Install failed, roll back changes
				$this->parent->abort(JText::_('Addon').' '.JText::_('Install').': '.JText::_('Addon').' "'.$pname.'" '.JText::_('already exists!'));
				return false;
			}
		...
		if (!$this->parent->copyManifest(-1)) {
			// Install failed, rollback changes
			$this->parent->abort(JText::_('Addon').' '.JText::_('Install').': '.JText::_('Could not copy setup file'));
			return false;
		}
