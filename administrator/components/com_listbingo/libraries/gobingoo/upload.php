			...
			if ($uploader->file_is_image) {
				...
				if (count ( $params->thumbnails ) > 0) {
					...
					foreach ( $params->thumbnails as $thumbnail ) {
						...
						$ratio = isset ( $thumbnail->ratio ) ? $thumbnail->ratio : true;
						$imagex = isset ( $thumbnail->x ) ? $thumbnail->x : true;
						$imagey = isset ( $thumbnail->y ) ? $thumbnail->y : true;
						...
						$uploader->image_ratio_crop = $crop;
